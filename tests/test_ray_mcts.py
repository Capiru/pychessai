...
def test_mcts(env,right_move):
    ...
    mcts = MCTS(model, {"temperature":1.0,"dirichlet_epsilon":0.25,"dirichlet_noise":0.25,"num_simulations":5000,"argmax_tree_policy":True,"argmax_child_value":True,"add_dirichlet_noise":True,
...
def test_mcts_match():
    model1 = LeelaZero(obs_space = spaces.Box(
                            low=0, high=1, shape=(8, 8, 111), dtype=bool
                        ),action_space = spaces.Box(
                            low=0, high=1, shape=(4672,), dtype=np.int8
                        ),num_outputs = 1,model_config = {},name = "LeelaZero")
    mcts1 = MCTS(model1, {"temperature":1.0,"dirichlet_epsilon":0.25,"dirichlet_noise":0.25,"num_simulations":100,"argmax_tree_policy":True,"argmax_child_value":True,"add_dirichlet_noise":True,
            "puct_coefficient":np.sqrt(2),"epsilon":0,"turn_based_flip":True})
    model2 = LeelaZero(obs_space = spaces.Box(
                            low=0, high=1, shape=(8, 8, 111), dtype=bool
                        ),action_space = spaces.Box(
                            low=0, high=1, shape=(4672,), dtype=np.int8
                        ),num_outputs = 1,model_config = {},name = "LeelaZero")
    mcts2 = MCTS(model2, {"temperature":1.0,"dirichlet_epsilon":0.25,"dirichlet_noise":0.25,"num_simulations":100,"argmax_tree_policy":True,"argmax_child_value":True,"add_dirichlet_noise":True,
            "puct_coefficient":np.sqrt(2),"epsilon":1,"turn_based_flip":True})
    ...
    print(env2.env.board.turn,env2.env.board.fen())
    half_turns = 0
    while not env2.env.game_over:
        if half_turns % 2 == 0:
            current_mcts = mcts1
        else:
            current_mcts = mcts2
        current_state = env2.get_state()
        tree_node = Node(
                            state=current_state,
                            obs=env2.observe(),
                            reward=0,
                            ...
                            action=None,
                            parent=RootParentNode(env=env2,state = current_state),
                            mcts=current_mcts,
                        )
        mcts_policy, action, tree_node = current_mcts.compute_action(tree_node)
        move = chess_utils.actions_to_moves[action]
        env2.set_state(current_state)
        #assert ch.Move.from_uci(move) in list(env2.env.board.legal_moves)
        env2.step(action)
        print(env2.env.board.turn,action)
        half_turns += 1
    print(env2.env.board.turn)
